const { getReviewsByUser, getReviews } = require('../db');


const getRestaurants = (restaurants, user_id=null) => {
    ...
    const sortedByUser = sortedByRating.map(restaurant => {
        const userReview = userReviews.find(review => review.restaurant_id === restaurant.id);
        return {
            ...restaurant,
            userReview
        }
    ...
    const bestForUser = sortedByUser.filter(restaurant => restaurant.userReview && restaurant.userReview.rating > 3);
    ...
    return merged.map(restaurant => {
        const rating = restaurant.reviews.reduce((acc, review) => acc + review.rating, 0) / restaurant.reviews.length;
        return {
            ...restaurant,
